...
private:
	...
	void createGraphicsPipeline() {
		...
		colorBlending.logicOpEnable = VK_FALSE; // VK_TRUE?
	...
	void createSwapChain() {
		...
		if (indices.graphicsFamily != indices.presentFamily) {
			std::cout << "Swap chain will use concurrent mode\n";
			createInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
			createInfo.queueFamilyIndexCount = 2;
			createInfo.pQueueFamilyIndices = queueFamilyIndicies;
		}
		else {
			std::cout << "Swap chain will use exclusive mode\n";
	...
	void pickPhysicalDevice() {
		...
		// Ordered map to automatically sort candidates by score
	...
	int rateDeviceSuitability(VkPhysicalDevice device) {
		...
		std::cout << "Found device " << deviceProperties.deviceName << " with score " << score << std::endl;
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
		...
		// If the specified format and colorspace is not found, return the first format. Should be enough
	...
	VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
		...
		// If MAILBOX_KHR is not found, return this mode because it is garanteed to be available
	...
	static std::vector<char> readFile(const std::string& filename) {
		...
		if (!file.is_open()) {
			char errorStr[1024];
			sprintf_s(errorStr, "Failed to load file %s!", filename.c_str());
		...
		std::cout << "Successfully loaded file " << filename << " of size " << fileSize << std::endl;
